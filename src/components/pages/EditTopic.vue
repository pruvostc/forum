      ...
      <div class="col-md-12">
        <form method="post" v-on:submit.prevent="editTopic">
          ...
          <button type="submit" class="btn btn-success">
            <i
              v-show="isLoading"
              class="fa fa-refresh fa-lg fa-spin btn-load-indicator"
            ></i>
            Save
          </button>
        </form>
...
export default {
  ...
  created() {
    ...
    api.getTopicByKey(this.$route.params.topic_id, function (
      topic
     // user,
     // forum
    ) {
  ...
  methods: {
    editTopic() {
      ...
      // else save topic
      api.updateTopic(
        self.$store.state.topic.title,
        self.$store.state.topic.content,
        self.$route.params.topic_id
      );
